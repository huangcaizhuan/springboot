...
+function ($) {
    ...
    Lookup.prototype.addBtn = function() {
        ...
        if (!this.$lookBtn && !$element.parent().hasClass('wrap_bjui_btn_box')) {
			...
			if(this.$element.hasClass('wpcen')){
				$box.css({'position':'relative', 'display':'block'})
			} else {$box.css({'position':'relative', 'display':'inline-block'})}
    ...
    Lookup.prototype.setVal = function(args, type) {
        ...
        if ($currentLookup.data('customEvent')) {
            ...
            $box.find('div').each(function() {
                var $div = $(this), divName = $div.attr('name')
                ...
                for (var key in args) {
                    ...
                    if (name == divName) {
                        ...
                        /* @description 增加 追加参数 @author 小策一喋 */
                        if(type == 1)
                            newValue = $div.text() ? $div.text() + ',' + args[key] : args[key]
                        else
                            newValue = args[key]

                        $div
                            .text(newValue) /* @description 修改 args[key] 为 newValue @author 小策一喋 */
                            .trigger(Lookup.EVENTS.afterChange, args)
