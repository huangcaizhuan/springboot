...
function modifyManager_submitcallback(json){
	if(!json.success){
		$.CurrentDialog.find('#modifyManager_form').alertmsg('error', json.msg);
		return;
	}
	$("#modifyManager_js-cancel").click();
	$(this).navtab('refresh');
}
var _index = 0;
//提交数据
/* $.CurrentDialog.find('#modifyManager_submieData').on('click',function(){
	var baseinfo = $.CurrentDialog.find('#modifyManager_form').serializeObject();
	var citys = '';
...
<div class="bjui-pageContent">
   	<form id="modifyManager_form" class="pageForm" data-toggle="validate"  data-loadingmask="false"  data-toggle="ajaxform" 
    	action="./modifyManagerAjax" method="post" data-alertmsg="false" data-loadingmask="false"  data-callback="modifyManager_submitcallback" >
                    ...
                    <tbody>
                        <tr>    
                            <td >
                                ...
                                <input type="text" name="name"  value="${manager.name }" data-rule="required" size="20">
                                
                                <label for="email" class="control-label x75">邮箱：</label>
                                <input type="text" name="email"  value="${manager.email }" data-rule="email" size="20">
                            </td>
                        </tr>
                        <tr>    
                             <td>
                             	<label for="tel" class="control-label x75">电话：</label>
                             	<input type="text" name="tel"  value="${manager.tel }" data-rule="mobile" size="20">
                            </td>
                        </tr>
                        <tr>
                         	<td>
                         		<label for="userName" class="control-label x75">登录名：</label>
                                <input type="text" name="userName"  value="${manager.userName }" data-rule="required" size="20">
...
<div class="bjui-pageFooter">
    <ul>
        <li><button type="button" class="btn-close" data-icon="close" id="modifyManager_js-cancel">取消</button></li>
        <li><button type="submit" class="btn-green" data-icon="save" id="modifyManager_submieData">保存</button></li>
    </ul>
</div>